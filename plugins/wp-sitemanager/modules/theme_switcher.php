 ...
 * Module Description:	Select thema by determine device 
 * Order:		100
 ...
 * 
 * TODO
 * * Add object caching to where the SQL statement is issued
 */
...
class theme_switcher {
    ...
    private function tambahan() {
        $vers = array(
            'module' => __('Theme Switcher', 'wp-sitemanager'),
            'Description' => __('Select thema by determine device', 'wp-sitemanager')
        );
    }
    ...
    public function add_setting_menu() {
        add_submenu_page($this->parent->root, __('Multi Device', 'wp-sitemanager'), __('Multi Device', 'wp-sitemanager'), 'administrator', basename($this->parent->root) . '-device', array(&$this, 'setting_page_controller'));
    }
...
//(2, 'スマートフォン', '', 'smart', 100, '0')";
            $sql = "
INSERT INTO `{$this->group_table}` (`group_id`, `group_name`, `theme`, `slug`, `priority`, `builtin`) VALUES
(1, 'Tablet', '', 'tablet', 200, '0'),
(2, 'Smart Phone', '', 'smart', 100, '0')";
            $wpdb->query($sql);
        }
...
(2, 8)";
    ...
    public function setting_page() {
        ...
        <div class="wrap">
            ...
            <h2><?php _e('Device settings', 'wp-sitemanager'); ?></h2>
            <h3><?php _e('Device Group', 'wp-sitemanager'); ?> <a href="<?php echo esc_url($add_group_url); ?>" class="button"><?php _e('Add new group', 'wp-sitemanager'); ?></a></h3>
            <?php $groups->display(); ?>
            <h3><?php _e('Device', 'wp-sitemanager'); ?> <a href="<?php echo esc_url($add_device_url); ?>" class="button"><?php _e('Add new device', 'wp-sitemanager'); ?></a></h3>
            <?php $items->display(); ?>
    ...
    function edit_group_page($id = false) {
        ...
        <div class="wrap">
            <?php screen_icon('prime-icon32'); ?>
            <h2><?php _e('Device Group', 'wp-sitemanager'); ?>
                <a href="<?php echo esc_url($list_page_url); ?>" class="button"><?php _e('Back to list', 'wp-sitemanager'); ?></a>
            </h2>
            <form action="" method="post">
                <table class="form-table">
                    <tr>
                        <th><?php _e('Group Name', 'wp-sitemanager'); ?></th>
                        <td><input type="text" name="group_name" size="30" value="<?php echo isset($group->group_name) ? esc_html($group->group_name) : ''; ?>" /></td>
                    </tr>
                    <tr>
                        <th><?php _e('Theme', 'wp-sitemanager'); ?></th>
                        <td>
                            <select name="theme">
                                <option value=""><?php _e('No Theme Switching', 'wp-sitemanager'); ?></option>
                                <?php
...
ORDER BY d.device_id ASC
        ...
        class SiteManager_Device_Group_List_Table extends WP_List_Table {
            ...
            function get_columns() {
                $columns = array(
                    'group_name' => __('Group Name', 'wp-sitemanager'),
                    'theme' => __('Theme', 'wp-sitemanager'),
                    'slug' => __('Slug', 'wp-sitemanager'),
                    'priority' => __('Priority', 'wp-sitemanager'),
                    'devices' => __('Device', 'wp-sitemanager')
                );
                return $columns;
            }
...
//		echo '編集 | 削除</p>';
        ...
        class SiteManager_Device_List_Table extends WP_List_Table {
            ...
            function get_columns() {
                $columns = array(
                    'device' => __('Device Name', 'wp-sitemanager'),
                    'keyword' => __('Meta Keywords', 'wp-sitemanager'),
                    'group' => __('Group Name', 'wp-sitemanager'),
                );
                return $columns;
            }
