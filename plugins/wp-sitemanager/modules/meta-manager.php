<?php
/*
 * cms module:		Meta Information Setting
 * Module Description:	Display meta keywords, meta description and SNS tags in the head tag.
 * Order:		10
 * First Introduced:	x
 * Major Changes In:	
 * Builtin:		true
...
class meta_manager {
    ...
    private function tambahan() {
        $vers = array(
            'module' => __('Meta Information Setting', 'wp-sitemanager'),
            'Description' => __('Display meta keywords, meta description and SNS tags in the head tag.', 'wp-sitemanager')
        );
    }
...
//				add_action( 'delete_' . $taxonomy, array( &$this, 'delete_term_meta' ) );
    ...
    function add_keywords_form() {
        ...
        <div class="form-field">
            <label for="meta_keywords"><?php _e('Meta Keywords', 'wp-sitemanager'); ?></label>
            <textarea name="meta_keywords" id="meta_keywords" rows="3" cols="40"></textarea>
        </div>
        <div class="form-field">
            <label for="meta_description"><?php _e('Meta Description', 'wp-sitemanager'); ?></label>
    ...
    function edit_keywords_form($tag) {
        ?>
        <tr class="form-field">
            <th scope="row" valign="top"><label for="meta_keywords"><?php _e('Meta Keywords', 'wp-sitemanager'); ?></label></th>
        ...
        <tr class="form-field">
            <th scope="row" valign="top"><label for="meta_description"><?php _e('Meta Description', 'wp-sitemanager'); ?></label></th>
    ...
    function add_post_meta_box($post_type, $post) {
        if (isset($post->post_type) && in_array($post_type, get_post_types(array('public' => true))) && $post_type != 'attachment') {
            add_meta_box('postmeta_meta_box', __('Meta Information', 'wp-sitemanager'), array(&$this, 'post_meta_box'), $post_type, 'normal', 'high');
        }
    }
    ...
    function output_meta() {
        ...
        if (is_home() || is_front_page()) {
            ...
            if (post_password_required())
                $meta['description'] = 'This post is password protected.';
        }
    ...
    private function get_ogp($meta) {
        ...
        if (empty($og_tags['og:title']))
            $og_tags['og:title'] = apply_filters('wp_sitemanager_open_graph_title', __('(no title)', 'wp-sitemanager'));
    ...
    function setting_page() {
        ...
        <div class="setting-section">
            <h3><?php _e('Keyword &amp; description', 'wp-sitemanager'); ?></h3>
            <h4><?php _e('Basic Setting', 'wp-sitemanager'); ?></h4>
            <table class="form-table">
                <tr>
                    <th><?php _e('Common Keyword', 'wp-sitemanager'); ?></th>
                    <td>
                ...
                <tr>
                    <th><?php _e('Basic Description', 'wp-sitemanager'); ?></th>
                    <td>
            ...
            <h4><?php _e('Post setting', 'wp-sitemanager'); ?></h4>
            <table class="form-table">
                <?php if ($taxonomies) : $cnt = 1; ?>
                    <tr>
                        <th><?php _e('Include taxonomies for keywords of post', 'wp-sitemanager'); ?></th>
                        <td>
                ...
                <?php endif; ?>
                <tr>
                    <th><?php _e('Description of post', 'wp-sitemanager'); ?></th>
                    <td>
                        <label for="excerpt_as_description">
                            ...
                            <?php _e('Using excerpt for post description', 'wp-sitemanager'); ?>
                        </label>
                    </td>
                </tr>
            </table>
            <h4><?php _e('Taxonomy Setting', 'wp-sitemanager'); ?></h4>
            <table class="form-table">
                <tr>
                    <th><?php _e('Meta Keywords Taxonomy', 'wp-sitemanager'); ?></th>
                    <td>
                        <label for="include_term">
                            ...
                            <?php _e('Include taxonomies name for keywords of taxonomy', 'wp-sitemanager'); ?>
                        </label>
                    </td>
                </tr>
            </table>
            <h4><?php _e('SNS Setting', 'wp-sitemanager'); ?></h4>
            <table class="form-table">
                <tr>
                    <th><?php _e('Basic Image', 'wp-sitemanager'); ?></th>
                    <td>
                        <button id="select-og-image" class="button"><?php _e('Select a picture', 'wp-sitemanager'); ?></button>

                        <button id="deleat-og-image" class="button"<?php echo empty($ogp_image_src) ? ' style="display: none"' : ''; ?>><?php _e('Remove Image', 'wp-sitemanager'); ?></button>
                        <div id="og-image">
                ...
                <tr>
                    <th><?php _e('Output of OGP', 'wp-sitemanager'); ?></th>
                    <td>
                        <label for="ogp_output">
                            ...
                            <?php _e('Output', 'wp-sitemanager'); ?>
                        </label>
                    </td>
                </tr>
                <tr>
                    <th><?php _e('Output of Twitter Cards', 'wp-sitemanager'); ?></th>
                    <td>
                ...
                <tr>
                    <th><?php _e('Twitter Account', 'wp-sitemanager'); ?></th>
                    <td>
    ...
    function setting_page_scripts() {
        ...
        wp_enqueue_script(
                'ogp-media-uploader', plugin_dir_url(dirname(__FILE__)) . 'js/media-uploader.js',
//		plugins_url( 'media-uploader.js', __FILE__),
                array('jquery'), filemtime(dirname(dirname(__FILE__)) . '/js/media-uploader.js'), false
        );
    }
