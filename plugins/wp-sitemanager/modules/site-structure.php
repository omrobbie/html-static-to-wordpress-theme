 ...
 * Module Description:	Management sitemap, breadcrumb navigation, and Sub Navigation.
 * Order:		0
...
class site_structure {
    ...
    function __construct($parent) {
        // Set the properties of the object infinity cms
        $this->parent = $parent;
        ...
        // Instantiation of the category page class 
        new category_page;
        // Site Map for short code 
        add_shortcode('sitemap', array(&$this, 'sitemap'));
        // Registration of sub-navigation widget 
        add_action('widgets_init', array(&$this, 'register_subnavi_widget'));
    }

    private function tambahan() {
        $vers = array(
            'module' => __('Site Structure', 'wp-sitemanager'),
            'Description' => __('Management sitemap, breadcrumb navigation, and Sub Navigation.', 'wp-sitemanager')
        );
    }
    ...
    public function add_sitemap_page() {
        add_submenu_page($this->parent->root, __('Site Structure', 'wp-sitemanager'), __('Site Structure', 'wp-sitemanager'), 'administrator', basename($this->parent->root) . '-structure', array(&$this, 'setting_page'));
    }

    /*
     * View of the site map settings page 
     * @since 0.0.1
     */
    ...
    public function setting_page() {
        ...
        <div class="wrap">
            ...
            <h2><?php _e('Sitemap Settings', 'wp-sitemanager'); ?></h2>
            <p><?php _e('Integrated management for configuration website structure, sitemap, breadcrumb, and sub navigation', 'wp-sitemanager'); ?></p>

            <form action="" method="post">
                <?php wp_nonce_field('wp-sitemanager-sitemap'); ?>
                <h3><?php _e('Website Structure Settings', 'wp-sitemanager'); ?></h3>
                <p><?php _e('Setting fixed page to be assigned each post type', 'wp-sitemanager'); ?></p>
                <table class="form-table">
                    ...
                    foreach ($post_types as $post_type) :
                        ...
                        <tr>
                            ...
                            <td>
                                <?php wp_dropdown_pages('sort_column=menu_order, post_title&name=site_structure[' . $post_type->name . '][page]&show_option_none='.__('Front page','wp-sitemanager') . $selected); ?>
                                <?php _e('set as','wp-sitemanager');?>
                                <select name="site_structure[<?php echo esc_attr($post_type->name); ?>][object]">
                                    <?php
                                    if (is_post_type_hierarchical($post_type->name)) :
                                        ?>
                                        <option value="archive"><?php _e('List', 'wp-sitemanager'); ?></option>
                                        <?php
                                    endif;
                                ...
                                <?php //_e('Characterize', 'wp-sitemanager'); ?>
                                <!--
                ...
                <h3><?php _e('Sitemap Settings', 'wp-sitemanager'); ?></h3>
                <p><?php _e('Setting for Sitemap', 'wp-sitemanager'); ?></p>

                <table class="form-table">
                    <tr>
                        <th><?php _e('Output levels', 'wp-sitemanager'); ?></th>
                        <td>
                            <select name="sitemap[disp_level]" id="sitemap_disp_level">
                                <option value="0"><?php _e('No limit', 'wp-sitemanager'); ?></option>
                                <?php
                                for ($i = 1; $i <= 10; $i++) :
                                    $selected = $this->settings['sitemap']['disp_level'] == $i ? ' selected="selected"' : '';
                                    ?>
                                    <option value="<?php echo esc_attr($i); ?>"<?php echo $selected; ?>><?php printf(esc_attr__('Level %d', 'wp-sitemanager'),$i); ?></option>
                                    <?php
                                endfor;
                                ?>
                            </select>
                    ...
                    -->
                    <tr>
                        <th><?php _e('Change style', 'wp-sitemanager'); ?></th>
                        <td>
                            <select name="sitemap[style]" id="sitemap_style">
                                <option value="" selected="selected"><?php _e('No style', 'wp-sitemanager'); ?></option>
                                <?php
                                if ($sitemap_styles) : foreach ($sitemap_styles as $slug => $sitemap_style) :
                ...
                <div id="sitemap_setting_explain">
                    <p><?php _e('For displaying sitemap, please describe “<code>[sitemap]</code>” to the section you want to view', 'wp-sitemanager'); ?></p>
                </div>

        <?php submit_button(__('Save Changes','psi_pm'), 'primary', 'wp-sitemanager-sitemap'); ?>
            </form>
    ...
    /*
     * Data update process of the site map settings 
     */
    ...
    /*
     * I want to add a check box in the site map to the public exclusion box post screen 
     */

    public function add_exclude_sitemap_checkbox() {
        global $post, $post_type;
        // Check hierarchy of whether valid 
        ...
        <?php _e('Excluding from sitemap display', 'wp-sitemanager'); ?>
            </label>
        </div>
        <?php
    }
    ...
    /*
     * Save update the site map exclusions article 
     */

    public function update_exclude_sitemap_setting($post_id, $post) {
        // You do not run in the case of such as auto save or post type hierarchy is not enabled. 
        if (!is_post_type_hierarchical($post->post_type) || defined('DOING_AUTOSAVE') && DOING_AUTOSAVE && !(int) $post_id) {
            return;
        }

        $current = get_post_meta($post_id, '_exclude_sitemap', true);
        if ($current) {
            //Remove configuration settings have been, 0 When the POST 
            if (isset($_POST['exclude_sitemap']) && !$_POST['exclude_sitemap']) {
                delete_post_meta($post_id, '_exclude_sitemap');
            }
        } else {
            // To update the configuration settings has not been, 1 When the POST 
            if (isset($_POST['exclude_sitemap']) && $_POST['exclude_sitemap']) {
    ...
    /*
     * I want to determine whether site map exclusion article 
     */
    ...
    /*
     * I get all the id of the article that are excluded set 
     */
...
";
    ...
    /*
     * Hook Add Site map exclusions term 
     */
    ...
    /*
     * Site Map exclusions to the classification Add New screen 
     */
    ...
    public function edit_sitemap_exclude_term_checkbox($tag) {
        ...
        ?>
        <tr class="form-field">
            <th scope="row" valign="top"><?php _e('Sitemap','wp-sitemanager');?></th>
            <td>
                <input type="hidden" name="sitemap_exclude_checkbox" value="0" />
                <label for="sitemap_exclude_checkbox">
                    <input type="checkbox" name="sitemap_exclude_checkbox" id="sitemap_exclude_checkbox" 
                           value="1" style="width: auto;" <?php echo in_array($tag->term_id, $ex_cats) ? ' checked="checked"' : ''; ?> />
                    <?php _e('Excluding from sitemap display','wp-sitemanager');?>
                </label>
            </td>
        </tr>
    ...
    /*
     * I want to save the term id of the site map exclusion 
     */

    public function update_sitemap_exclude_terms($term_id) {
        if (!isset($_POST['sitemap_exclude_checkbox'])) {
            return;
        }
        $ex_cats = get_option('infinity_exclude_terms', array());
    ...
    /*
     * Update of exclusions of term when deleting 
     */
    ...
    /*
     * I should not be rewritten web.cofig of file there is a description of the location 
     */
...
class category_page {
    ...
    function __construct() {
        ...
        // Filtering to that of the child page permalinks fixed page that is set category page 
        add_filter('page_link', array(&$this, 'replace_category_page_permalink'), 10, 2);

        // Filtering to that of the child page permalinks of custom post type that is set category page 
        add_filter('post_type_link', array(&$this, 'replace_category_page_permalink'), 10, 2);

        // Redirection processing when it is accessed category page 
        add_action('template_redirect', array(&$this, 'redirect_category_page'));

        // Empty the contents of the feed of the category page 
    ...
    /*
     * I do determine if category page 
     * @since 0.0.1
     * @param int
     * @return bool
     */
    ...
    /*
     * The additional output the check box for the category page setup for the public posting of the screen box 
     * @since 0.0.1
     */
    ...
    public function add_category_page_checkbox() {
        ...
        <div class="misc-pub-section misc-pub-section-last">
            ...
            <label for="category_page_checkbox">
                ...
                <?php _e('Set as category page, and change permalink to first child page.','wp-sitemanager');?>
            </label>
        </div>
        <?php
    }
    ...
    /*
     * Permalink process of changing the category page 
     * @since 0.0.1
     */

    public function replace_category_page_permalink($link, $id_or_post) {
        // The absorption parameters differences and to determine (in the case of custom post type) or object (in the case of fixed page) id 
        ...
        // Run the link rewrite only if the hierarchy is enabled 
        if (is_post_type_hierarchical($post->post_type)) {
            if ($this->is_category_page($post->ID)) {
                // Get the permalink of child page 
    ...
    /*
     * Redirect processing of category page 
     * @since 0.0.1
     */

    public function redirect_category_page() {
        global $post;
        // Hierarchy determines whether enabled by individual display 
        if (is_singular() && is_post_type_hierarchical($post->post_type)) {
            if ($this->is_category_page($post->ID)) {
                // To perform a redirect if you can get a permanent link of the child page. 
    ...
    /*
     * Get the child the first page 
     * @since 0.0.1
     * 
     * return :object Object of the child page 
     */
    ...
    /*
     * Get the permalink of a child the first page 
     * @since 0.0.1
     * @param object Object of the current page 
     * @return string Url of the child page 
     */
    ...
    /*
     * In the case of category page, empty the contents of a feed 
     * @since 0.0.1
     * @param string Display the contents of the default 
...
/*
 * I will do the class definitions for (Sabunabi) and site map. 
 * The class inherits the (wp-includes/post-template.php) Walker_Page class that generates the html of wp_list_pages, 
 * I have override some methods. 
 * It has inherited Walker class (wp-includes/class-wp-walker.php), Walker_Page class has a recursive basic logic. 
 * The basic operation, by walker method, it is classified as child items and the top item, we are the source generated by display_element method. 
 * In display_element inside, if there is an item in the child, and has realized the recursion behavior by calling display_element recursively. 
 */
...
class Walker_pageNavi extends Walker_Page {
    ...
    function walk($elements, $max_depth) {
        ...
        /*
         * Here, because the logic (Show child page the parent is hidden specified) stray, should you do not need a site map.
         * 
    ...
    /*
     * View custom post type hanging in fixed page in the site map, the tree (category, Tagu含) of custom classification
     */
    ...
    function display_post_type_tree($post_type, $output, $depth, $args) {
        ...
        $custom_structure = get_option('wp-sitemanager-site-structure');
        if (!isset($custom_structure[$post_type]['object'])) {
            return;
        }
...
class infinity_sub_navi_widget extends WP_Widget {
    ...
    function infinity_sub_navi_widget() {
        $widget_ops = array(
            ...
            'description' => __('Realize an all-in-one sub navigation for home, post, and fixed page','wp-sitemanager')
        );
        ...
        $this->WP_Widget('sub_navi', __('Sub Navigation','wp-sitemanager'), $widget_ops, $widget_ops);
    }

    function widget1($args, $instance) {
        global $post;
...
//		var_dump( $args, $instance );
        ...
        if (is_home()) {
            ...
            switch ($instance['home_display']) {
                case 'latest' :
                    $orderby = 'date';
                    $widget_title = __('Latest News','wp-sitemanager');
                    break;
                case 'modify' :
                    $orderby = 'modified';
                    $widget_title = __('Update News','wp-sitemanager');
                    break;
                default :
                    return;
            }
...
//		var_dump( $args, $instance );
        ...
        if (is_home()) {
            switch ($instance['home_display']) {
                case 'latest' :
                    $orderby = 'date';
                    $widget_title = 'Informasi Terkini';
                    break;
                case 'modify' :
                    $orderby = 'modified';
                    $widget_title = 'diperbarui';
                    break;
                default :
                    return;
            }
            ...
            switch ($instance['post_display']) {
                ...
                case 'y_archives' :
                    $archive_type = 'yearly';
                    $widget_title = __('Archives', 'all_in_one_sub_navi_widget');
                    add_filter('get_archives_link', array(&$this, 'add_nen_for_get_archives'));
                    break;
                case 'm_archives' :
                    $archive_type = 'monthly';
            ...
            /*
              ...
              $child_of = 0;
              $taxonomy_object = get_taxonomy( $taxonomy );
              $widget_title = $taxonomy == 'category' ? __( 'Category','all_in_one_sub_navi_widget' ) : $taxonomy_object->label;
              break;
              case 'root' :
              $ancestors = get_ancestors( $current->term_id, $taxonomy );
              if ( count( $ancestors ) ) {
...
//		var_dump( $instance );
        ...
        <h5><?php _e('Home','wp-sitemanager');?></h5>
        <p><?php _e('Display content','wp-sitemanager');?>：<br />
            <select name="<?php echo $this->get_field_name('home_display'); ?>">
                <option value="none"><?php _e('Not displayed','wp-sitemanager');?></option>
                <option value="latest"<?php echo $instance['home_display'] == 'latest' ? ' selected="selected"' : ''; ?>><?php _e('Latest News','wp-sitemanager');?></option>
                <option value="modify"<?php echo $instance['home_display'] == 'modify' ? ' selected="selected"' : ''; ?>><?php _e('Update News','wp-sitemanager');?></option>
            </select>
        </p>
        <p><?php _e('Title','wp-sitemanager');?>：<br />
            <input type="text" size="30" name="<?php echo $this->get_field_name('home_title'); ?>" value="<?php echo esc_html($instance['home_title']); ?>" />
        </p>
        <p><?php _e('Views','wp-sitemanager');?>：<br />
            <input type="text" size="2" name="<?php echo $this->get_field_name('home_disp_nums'); ?>" value="<?php echo esc_html($instance['home_disp_nums']); ?>" />
        </p>
        <p><?php _e('Post type','wp-sitemanager');?>：<br />
        ...
        <h5><?php _e('Post and Archives','wp-sitemanager');?></h5>
        <p><?php _e('Display content','wp-sitemanager');?>：<br />
            <select name="<?php echo $this->get_field_name('post_display'); ?>">
                <option value="none"><?php _e('Not displayed','wp-sitemanager');?></option>
                <option value="all_cat"<?php echo $instance['post_display'] == 'all_cat' ? ' selected="selected"' : ''; ?>><?php _e('All categories','wp-sitemanager');?></option>
                <option value="root_cat"<?php echo $instance['post_display'] == 'root_cat' ? ' selected="selected"' : ''; ?>><?php _e('Children of the root category','wp-sitemanager');?></option>
                <option value="current_cat"<?php echo $instance['post_display'] == 'current_cat' ? ' selected="selected"' : ''; ?>><?php _e('Children of current category','wp-sitemanager');?></option>
                <option value="y_archives"<?php echo $instance['post_display'] == 'y_archives' ? ' selected="selected"' : ''; ?>><?php _e('Yearly Archives','wp-sitemanager');?></option>
                <option value="m_archives"<?php echo $instance['post_display'] == 'm_archives' ? ' selected="selected"' : ''; ?>><?php _e('Monthly Archives','wp-sitemanager');?></option>
            </select>
        </p>
        <p><?php _e('Title','wp-sitemanager');?>：<br />
            <input type="text" size="30" name="<?php echo $this->get_field_name('post_title'); ?>" value="<?php echo esc_html($instance['post_title']); ?>" />
        </p>
        <p><?php _e('Category display order ','wp-sitemanager');?>：<br />
            <select name="<?php echo $this->get_field_name('cat_orderby'); ?>">
                <option value="name"<?php echo $instance['cat_orderby'] == 'name' ? ' selected="selected"' : ''; ?>><?php _e('Category Name','wp-sitemanager');?></option>
                <option value="slug"<?php echo $instance['cat_orderby'] == 'slug' ? ' selected="selected"' : ''; ?>><?php _e('Category slug','wp-sitemanager');?></option>
                <option value="count"<?php echo $instance['cat_orderby'] == 'count' ? ' selected="selected"' : ''; ?>><?php _e('Number of posts','wp-sitemanager');?></option>
            </select>
            <select name="<?php echo $this->get_field_name('cat_order'); ?>">
                <option value="asc"<?php echo $instance['cat_order'] == 'asc' ? ' selected="selected"' : ''; ?>><?php _e('Ascending','wp-sitemanager');?></option>
                <option value="desc"<?php echo $instance['cat_order'] == 'desc' ? ' selected="selected"' : ''; ?>><?php _e('Descending','wp-sitemanager');?></option>
            </select>
        </p>

        <p><?php _e('Archive Views','wp-sitemanager');?>：<br />
            <input type="text" size="2" name="<?php echo $this->get_field_name('post_disp_nums'); ?>" value="<?php echo esc_html($instance['post_disp_nums']); ?>" />
        </p>

        <h5><?php _e('Fixed page','wp-sitemanager');?></h5>
        <p><?php _e('Display content','wp-sitemanager');?>：<br />
            <select name="<?php echo $this->get_field_name('page_display'); ?>">
                <option value="none"><?php _e('Not displayed','wp-sitemanager');?></option>
                <option value="root"<?php echo $instance['page_display'] == 'root' ? ' selected="selected"' : ''; ?>><?php _e('View underlying page, based in root page','wp-sitemanager');?></option>
                <option value="root_current"<?php echo $instance['page_display'] == 'root_current' ? ' selected="selected"' : ''; ?>><?php _e('View first, top level, child pages, based in root page','wp-sitemanager');?></option>
                <option value="current"<?php echo $instance['page_display'] == 'current' ? ' selected="selected"' : ''; ?>><?php _e('View underlying page, based in current page','wp-sitemanager');?></option>
            </select><br />
            ...
            <label for="<?php echo $this->get_field_name('page_display_child_of');?>"><?php _e('View base page','wp-sitemanager');?></label>
        </p>
        <p><?php _e('Title','wp-sitemanager');?>：<br />
            <input type="text" size="30" name="<?php echo $this->get_field_name('page_title'); ?>" value="<?php echo esc_html($instance['page_title']); ?>" />
        </p>

        <p><?php _e('Level of display','wp-sitemanager');?>：<br />
        ...
        <p><?php _e('Exclusion page ID','wp-sitemanager');?>：<br />
